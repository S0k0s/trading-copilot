...
## 📈 Ιστορικό Επιδόσεων — έλεγχος αν το μοντέλο όντως δουλεύει

Νέο tab στην ενότητα «Ανάλυση». Κάθε scan (`snapshot_picks()` στο `scan.py`) καταγράφει σε
`picks_history.json` (append-only, κρατάει ~180 ημέρες) ποιες μετοχές ήταν εκείνη τη μέρα στο
Top-5 Long-Term/Swing του tab «Σήμερα», μαζί με την τιμή τους και την τιμή του SPY (proxy S&P 500)
ως σημείο αναφοράς. Το `assets/performance.js` συγκρίνει αργότερα την τότε τιμή με τη σημερινή του
`data.json` και υπολογίζει win rate / μέση απόδοση / alpha έναντι SPY, ανά ωρίμανση (~1 εβδ. /
~2 εβδ. / ~1 μήνα+). Καμία νέα εξωτερική πηγή δεδομένων — μόνο ό,τι ήδη υπολογίζουμε, με μνήμη.
Χρειάζεται μερικές μέρες συσσώρευσης πριν φανούν τα πρώτα ώριμα αποτελέσματα.
